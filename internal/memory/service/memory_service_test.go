...
func (m *MockStreamMemoryRepository) FindUnprocessed(ctx context.Context, before time.Time, limit int, opts *models.QueryOptions) ([]models.StreamMemory, error) {
	args := m.Called(ctx, before, limit, opts)
	return args.Get(0).([]models.StreamMemory), args.Error(1)
}
...
func (m *MockLongTermMemoryRepository) FindByDate(ctx context.Context, date string, opts *models.QueryOptions) (*models.LongTermMemory, error) {
...
func TestMemoryService_WriteMemory(t *testing.T) {
	...
	t.Run("重复 trace_id", func(t *testing.T) {
		...
		existing := []models.StreamMemory{
			{ID: 1, UserCode: "user-001", Date: now.Format("2006-01-02"), Content: "已有内容", SourceIDs: "trace-002"},
		}
...
func TestMemoryService_SearchMemory(t *testing.T) {
	...
	t.Run("关键词搜索", func(t *testing.T) {
		...
		streams := []models.StreamMemory{
			{ID: 1, UserCode: "user-001", Date: now.Format("2006-01-02"), Content: "包含关键词的内容", SourceIDs: "trace-001"},
		}
		...
		longTerms := []models.LongTermMemory{
			{ID: 1, Date: "2026-03-02", Summary: "关键词总结", CreatedAt: now},
		}
...
func TestMemoryService_UpgradeStreamToLongTerm(t *testing.T) {
	...
	t.Run("成功升级", func(t *testing.T) {
		...
		// 模拟查询流水记忆
		streams := []models.StreamMemory{
			{ID: 1, UserCode: "user-001", Date: date, Content: "内容1", SourceIDs: "trace-001"},
			{ID: 2, UserCode: "user-001", Date: date, Content: "内容2", SourceIDs: "trace-002"},
		}
		...
		// 检查是否存在长期记忆
		longTermRepo.On("FindByDate", ctx, date, mock.Anything).Return(nil, nil).Once()
...
func TestMemoryService_SearchMemory_WithFilters(t *testing.T) {
	...
	t.Run("只搜索长期记忆", func(t *testing.T) {
		now := time.Now()
		longTerms := []models.LongTermMemory{
			{ID: 1, Date: "2026-03-01", Summary: "总结1", CreatedAt: now},
		}
		longTermRepo.On("SearchByKeyword", ctx, "关键词", mock.Anything).Return(longTerms, nil).Once()
...
func TestMemoryService_UpgradeStreamToLongTerm_Existing(t *testing.T) {
	...
	longTermRepo.On("FindByDate", ctx, date, mock.Anything).Return(existing, nil).Once()
...
func TestMemoryService_FilterStreamByKeyword(t *testing.T) {
	...
	now := time.Now()
	streams := []models.StreamMemory{
		{ID: 1, UserCode: "user-001", Date: now.Format("2006-01-02"), Content: "Hello World", Summary: "问候"},
		{ID: 2, UserCode: "user-001", Date: now.Format("2006-01-02"), Content: "Go Programming", Summary: "编程"},
		{ID: 3, UserCode: "user-001", Date: now.Format("2006-01-02"), Content: "Hello Go", Summary: "Go语言"},
	}
	streamRepo.On("FindByTimeRange", ctx, mock.Anything, mock.Anything, mock.Anything).Return(streams, nil).Once()
...
func TestMemoryService_LimitValidation(t *testing.T) {
	...
	t.Run("默认限制", func(t *testing.T) {
		now := time.Now()
		streams := []models.StreamMemory{{ID: 1, UserCode: "user-001", Date: now.Format("2006-01-02"), Content: "内容"}}
		streamRepo.On("FindByTimeRange", ctx, mock.Anything, mock.Anything, mock.Anything).Return(streams, nil).Once()
