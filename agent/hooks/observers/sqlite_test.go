...
// setupTestDependencies 创建测试依赖
func setupTestDependencies(t *testing.T) (*database.Client, repository.EventRepository, service.ConversationService) {
	tmpDir := t.TempDir()
...
// setupTestConfig 创建测试配置（用于 DatabaseLocation 测试）
func setupTestConfig(t *testing.T) string {
	return t.TempDir()
}

func TestNewSQLiteObserver(t *testing.T) {
	dbClient, repo, convService := setupTestDependencies(t)
	defer dbClient.Close()

	// 创建观察器
	obs, err := NewSQLiteObserver(zap.NewNop(), nil,
		WithDBClient(dbClient),
		WithRepository(&EventRepositoryAdapter{Repo: repo}),
		WithConversationService(&ConversationServiceAdapter{Svc: convService}),
	)
	if err != nil {
...
func TestSQLiteObserver_OnEvent_PromptSubmitted(t *testing.T) {
	...
	result, err := convService.GetByTraceID(context.Background(), "trace-123")
	if err != nil {
		t.Fatalf("查询失败: %v", err)
	}
	if len(result) != 1 {
...
func TestSQLiteObserver_OnEvent_IgnoredEvents(t *testing.T) {
	...
	// 验证数据未插入（使用直接数据库查询）
	var count int64
	if err := dbClient.DB().Model(&models.Event{}).Count(&count).Error; err != nil {
		t.Fatalf("查询失败: %v", err)
	}
	if count != 0 {
...
func TestSQLiteObserver_Filter(t *testing.T) {
	...
	obs, err := NewSQLiteObserver(zap.NewNop(), filter,
		WithDBClient(dbClient),
		WithRepository(&EventRepositoryAdapter{Repo: repo}),
		WithConversationService(&ConversationServiceAdapter{Svc: convService}),
	)
...
// DBClientWrapper 包装 database.Client 以实现 DBClient 接口
// 注意：database.Client 已经实现了 DBClient 接口，此包装器仅用于文档说明
type DBClientWrapper = database.Client

// 确保 database.Client 实现 DBClient 接口
var _ DBClient = (*database.Client)(nil)
